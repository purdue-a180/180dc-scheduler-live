...
async function sendCandidateEmail(cand, ev, cohort, date, kind = "booked") {
  ...
  const locStr = [loc.building, loc.room, loc.address].filter(Boolean).join(", ");
...
function PasswordGateModal({ onResolve, message }) {
  ...
  return (
    <div className="modal-overlay" onClick={() => onResolve(false)}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="muted" style={{ marginTop: -6, marginBottom: 14 }}>{message || "This event is published. Enter the admin password to make this change."}</p>
...
function InterviewBooking({ data, onBook, onHoldSlot, onReleaseSlot, onHoldExpired, go }) {
  ...
  const loc = ev.location || {};
  const locStr = [loc.building, loc.room].filter(Boolean).join(" · ") || "Location TBA";
...
function CandidateConfirmation({ info, emailStatus, go }) {
  ...
  const locStr = [loc.building, loc.room, loc.address].filter(Boolean).join(", ") || "TBA";
    ...
    <div className="fadein">
      <div className="card form-card" style={{ maxWidth: 720 }}>
        ...
        <div className="field-row">
          ...
          <Field label="Room / Floor" value={f.room} onChange={set("room")} placeholder="3082 or 2nd Floor" />
        </div>
...
function IACandidates({ data, save, ev }) {
  ...
  const [confirmId, setConfirmId] = useState(null);      // which row is showing "are you sure?"
  const [pwGate, setPwGate] = useState(false);
  const pwResolveRef = useRef(null);
  if (!ev) return <EmptyEvents />;
  ...
  /* Deleting/cancelling a candidate is a one-way, hard-to-undo action, so it's
     behind three steps: open the ⋯ menu (not a bare button sitting in the row),
     an inline "are you sure?" confirm, and then the same admin password used
     elsewhere in this app — a real modal, not window.prompt (see the note on
     PasswordGateModal for why: native prompts are silently blocked in this
     preview and other sandboxed contexts). */
  const requestPassword = () => new Promise((resolve) => { pwResolveRef.current = resolve; setPwGate(true); });
  const resolvePw = (ok) => {
    setPwGate(false);
    const r = pwResolveRef.current; pwResolveRef.current = null;
    if (r) r(ok);
  };
  const confirmedCancel = async (id) => {
    setConfirmId(null);
  ...
  return (
    <div className="fadein">
      {pwGate && <PasswordGateModal onResolve={resolvePw} message="Cancelling a candidate's booking needs the admin password to prevent an accidental delete." />}
              ...
              {filtered.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="row-menu-cell">
                    {confirmId === c.id ? (
                      <div className="confirm-inline">
                        <span className="confirm-q">Cancel this booking?</span>
                        <Btn kind="danger" small onClick={() => confirmedCancel(c.id)}>Yes</Btn>
                        <Btn kind="outline" small onClick={() => setConfirmId(null)}>No</Btn>
                      </div>
                    ) : (
                      <div className="row-menu-wrap">
                        <button className="row-menu-btn" onClick={(ev2) => openMenu(c.id, ev2)} aria-label="Row actions">⋯</button>
                        {openMenuId === c.id && menuPos && (
                          <>
                            ...
                            <div className="row-menu" style={{ top: menuPos.top, right: menuPos.right }}>
                              <button className="row-menu-item danger" onClick={() => { setOpenMenuId(null); setConfirmId(c.id); }}>Cancel booking</button>
                            </div>
                          </>
                        )}
                      </div>
        ...
        {locForm && (
          <>
            <div className="field-row">
              ...
              <Field label="Room / Floor" value={locForm.room} onChange={(e) => setLocForm({ ...locForm, room: e.target.value })} placeholder="3082 or 2nd Floor" />
            </div>
...
.row-menu-cell { position: relative; text-align: right; }
.row-menu-wrap { position: relative; display: inline-block; }
.row-menu-btn { width: 30px; height: 30px; border-radius: 8px; border: 1px solid #E2E2DC; background: #fff;
  font-size: 18px; line-height: 1; color: #555; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; }
.row-menu-btn:hover { background: #F5F5F0; color: #222; }
.row-menu-backdrop { position: fixed; inset: 0; z-index: 90; background: transparent; }
.row-menu { position: fixed; z-index: 91; background: #fff; border-radius: 10px;
  box-shadow: 0 10px 26px rgba(0,0,0,.18); border: 1px solid #ECECE6; min-width: 150px; overflow: hidden; }
.row-menu-item { display: block; width: 100%; text-align: left; padding: 10px 14px; font-size: 13.5px; font-weight: 600;
  border: none; background: none; cursor: pointer; color: #333; }
.row-menu-item:hover { background: #F5F5F0; }
.row-menu-item.danger { color: #C0392B; }
.row-menu-item.danger:hover { background: #FDEEEC; }
.site.dark .row-menu-btn { background: #22261B; border-color: #333A28; color: #C6CABE; }
.site.dark .row-menu-btn:hover { background: #2A2F20; color: #F3F5EC; }
.site.dark .row-menu { background: #22261B; border-color: #333A28; }
.site.dark .row-menu-item { color: #E4E7DC; }
.site.dark .row-menu-item:hover { background: #2A2F20; }
.site.dark .row-menu-item.danger:hover { background: #3A231F; }
